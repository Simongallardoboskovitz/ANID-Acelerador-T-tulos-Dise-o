...
const InterviewGuideGenerator = ({ type, persona, onAccept, setIsAiTyping, allData }: {
    ...
    const { aiActive } = useContext(DevModeContext);
    ...
    const generateGuide = async (isRobustify = false) => {
        ...
        if (!aiActive) {
            setGuide(`<p>Guía de entrevista ${type} (IA Inactiva)</p><ul><li>Pregunta de ejemplo 1</li><li>Pregunta de ejemplo 2</li></ul>`);
            setIsLoading(false);
            setIsAiTyping(false);
            return;
        }
        
        const pdfContextPrompt = allData.pdfContent
            ? `\n\nContexto Adicional del Documento del Estudiante:\n${allData.pdfContent}`
            : '';
...
const UserResearch: React.FC<UserResearchProps> = ({ onAccept, profile, allData, setIsAiTyping }) => {
    ...
    const { devMode, aiActive } = useContext(DevModeContext);

    useEffect(() => {
        if (devMode) {
            const randomId = Math.floor(Math.random() * 1000);
            setNeeds(`Necesidad de prueba #${randomId}: herramientas de testeo más rápidas.`);
            setBehaviors(`Comportamiento de prueba #${randomId}: hace clic en todos los botones para probar.`);
            setPains(`Dolor de prueba #${randomId}: animaciones de texto lentas que bloquean el flujo.`);
        }
    }, [devMode]);
    ...
    useEffect(() => {
        const generateSummary = async () => {
            ...
            if (!aiActive) {
                setSummaryText('<strong>Resumen (IA inactiva):</strong><p>Por favor, completa los campos de usuario a continuación.</p>');
            ...
            try {
                const ai = new GoogleGenAI({ apiKey: process.env.API_KEY });
                const pdfContextPrompt = allData.pdfContent
                    ? `\n\n--- INICIO DEL DOCUMENTO DEL ESTUDIANTE ---\n${allData.pdfContent}\n--- FIN DEL DOCUMENTO DEL ESTUDIANTE ---\n`
                    : '';
...
**Tarea:** Redacta un texto consolidado que entrelace coherentemente los puntos clave de los datos proporcionados, incluyendo el documento del estudiante si existe. Usa **títulos en negrita** (ej. **Tema**) para separar cada sección. El texto debe ser editable por el usuario. No uses HTML, solo texto plano con markdown para negrita.`;
    ...
    const handleGeneratePersona = async (isIteration = false) => {
        ...
        if (!aiActive) {
            setNarrativePersona('<h3>User Persona (IA Inactiva)</h3><p>Este es un perfil de ejemplo. Puedes editarlo o aceptarlo para generar las guías de entrevista.</p>');
                     ...
                     {view === 'persona_accepted' && (
                         <div className="mt-6 border-t pt-6">
                            ...
                            {qualitativeGuide === null ? (
                                <InterviewGuideGenerator type="qualitative" persona={narrativePersona} onAccept={setQualitativeGuide} setIsAiTyping={setIsAiTyping} allData={allData} />
                            ) : (
                             ...
                             {quantitativeGuide === null ? (
                                <InterviewGuideGenerator type="quantitative" persona={narrativePersona} onAccept={setQuantitativeGuide} setIsAiTyping={setIsAiTyping} allData={allData} />
                            ) : (
                                <div className="bg-gray-100 border-l-4 border-black text-black p-4 mt-6 rounded">
