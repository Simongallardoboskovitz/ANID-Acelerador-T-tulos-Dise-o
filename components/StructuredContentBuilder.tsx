...
const StructuredContentBuilder: React.FC<StructuredContentBuilderProps> = ({
    ...
    useEffect(() => {
        if (devMode) {
            ...
            if (step === 7) { // Research Question
                setFormData({
                    intervention: `un conjunto de herramientas DevEx #${randomId}`,
                    aspect: 'la calidad y originalidad de las propuestas de título',
                    user: 'estudiantes de diseño en su etapa final',
                    context: 'en un taller con asistencia de IA'
                });
            } else if (step === 8) { // Hypothesis
                setFormData({
                    action: `se implementa un sistema de testeo automático #${randomId}`,
                    result: 'aumentará la velocidad de iteración del proyecto',
                    reason: 'la reducción de la carga cognitiva permite más enfoque creativo'
                });
            } else if (step === 9) { // General Objective
                setFormData({
                    verb: `Diseñar`,
                    what: `un sistema de desarrollo asistido por IA #${randomId}`,
                    purpose: 'mejorar la experiencia de desarrollo de los estudiantes de título'
                });
            } else if (step === 10 && onObjectivesChange) {
                onObjectivesChange([
                    `Investigar las necesidades de los desarrolladores #${randomId}`,
                    `Diseñar y prototipar una interfaz de testing #${randomId}`,
                    `Evaluar el impacto de la herramienta en la velocidad de desarrollo #${randomId}`
                ]);
            }
        }
    }, [devMode, step]);
    ...
    const pdfContextPrompt = allData.pdfContent
        ? `\n\n--- INICIO DEL DOCUMENTO DEL ESTUDIANTE ---\n${allData.pdfContent}\n--- FIN DEL DOCUMENTO DEL ESTUDIANTE ---\nUsa el contenido de este documento como el contexto principal del trabajo que el estudiante ya ha realizado.`
        : '';
    ...
    const handleImprove = async () => {
        ...
        if (!aiActive) {
            setImprovedText(`${composedText} (Mejora omitida, IA inactiva).`);
            setView('composed');
            setIsLoading('');
            setIsAiTyping(false);
            return;
...
Versión mejorada:`;
    ...
    const handleRobustify = async () => {
        ...
        if (!aiActive) {
            setRobustOptions(['Opción 1 (IA inactiva)', 'Opción 2 (IA inactiva)', 'Opción 3 (IA inactiva)']);
...
No agregues numeración ni títulos.`;
    ...
    const handleCorrectSyntax = async () => {
        ...
        if (!aiActive) {
            setCorrectedObjectives(objectives || []);
            setIsLoading('');
            setIsAiTyping(false);
            return;
        }
...
`;
    ...
    const handleRefineWithAI = async () => {
        ...
        if (!aiActive) {
            setRefineOptions([
                baseObjectives?.join('\n') || '',
                '1. Opción alternativa A (IA Inactiva)\n2. Opción alternativa B (IA Inactiva)\n3. Opción alternativa C (IA Inactiva)',
                '1. Opción diferente A (IA Inactiva)\n2. Opción diferente B (IA Inactiva)\n3. Opción diferente C (IA Inactiva)',
            ]);
            setIsLoading('');
            setIsAiTyping(false);
            return;
