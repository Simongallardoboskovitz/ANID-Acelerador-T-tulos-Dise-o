...
const PersonalContribution: React.FC<PersonalContributionProps> = ({ 
    ...
    useEffect(() => {
        if (devMode) {
            ...
            onContributionChange(`Mi aporte será crear un sistema que demuestre cómo una buena DevEx #${randomId} puede reducir la fricción y aumentar la creatividad en proyectos complejos.`);
        }
    }, [devMode]);
    ...
    const generateAiContent = async (isIteration = false) => {
        ...
        if (!aiActive) {
            onAiResponseChange('<p>Respuesta de IA omitida (IA inactiva). Este es un texto de ejemplo que puedes editar o aceptar.</p>');
            setIsLoading(false);
            setIsAiTyping(false);
            // onAnimationComplete will be called by HtmlTypewriter
            return;
        }
