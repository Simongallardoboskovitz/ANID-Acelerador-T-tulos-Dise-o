...
const WorkPlan: React.FC<WorkPlanProps> = ({ onAccept, profile, allData, setIsAiTyping }) => {
    ...
    useEffect(() => {
        if (devMode) {
            ...
            setTasks([
                { id: 1, text: `Implementar modo desarrollador #${randomId}` },
                { id: 2, text: `Testear el modo desarrollador #${randomId}` },
                { id: 3, text: `Documentar el modo desarrollador #${randomId}` },
            ]);
        }
    }, [devMode]);
    ...
    const handleGenerateActivities = async () => {
        ...
        if (!aiActive) {
            setKeyActivities('<ul><li>Actividad de ejemplo 1 (IA inactiva)</li><li>Actividad de ejemplo 2 (IA inactiva)</li></ul>');
            setIsActivitiesLoading(false);
            setIsAiTyping(false);
            // HtmlTypewriter will call onComplete to set animation done
            return;
        }
...
**Tarea:** Genera una lista HTML (<ul><li>) de actividades clave. Sé específico y accionable.`;
    ...
    const handleGeneratePlan = async () => {
        ...
        if (!aiActive) {
            setGanttChart('<h3>Plan de Trabajo (IA Inactiva)</h3><p>La generación del Gantt ha sido omitida.</p>');
            setIsLoading(false);
            setIsAiTyping(false);
            return;
        }
