...
const ProjectProposal: React.FC<ProjectProposalProps> = ({ onAccept, profile, allData, setIsAiTyping }) => {
    ...
    useEffect(() => {
        if (devMode) {
            ...
            setValueProp(`Mi propuesta de valor #${randomId} es un framework de testeo rápido que permite a los estudiantes validar ideas de proyectos de título en horas en lugar de semanas.`);
        }
    }, [devMode]);
    ...
    useEffect(() => {
        const generateIntroSummary = async () => {
            ...
            if (!aiActive) {
                setIntroSummary("<p>Resumen introductorio (IA inactiva). Por favor, escriba su propuesta de valor directamente.</p>");
...
Tarea: Crea un párrafo introductorio conciso que integre estos elementos, destacando en negrita (<strong>) los conceptos más importantes que deberían guiar la propuesta de valor. El objetivo es dar léxico útil, no resumir todo. Usa HTML.`;
    ...
    }, [allData.generalObjective, allData.projectGapAnalysis, allData.referenceAnalysis, allData.pdfContent]);
    ...
    const handleArticulate = async (isIteration = false) => {
        ...
        if (!aiActive) {
            setProposalPopupContent('Propuesta de proyecto generada (IA inactiva). Puede editar este texto.');
            setIsLoading(false);
            setIsAiTyping(false);
            return;
        }
...
Articula UNA ÚNICA propuesta de proyecto concisa y convincente. La propuesta debe integrar la "propuesta de valor" del estudiante con su objetivo y los referentes analizados, y el contexto del PDF. No debe superar las 140 palabras.
