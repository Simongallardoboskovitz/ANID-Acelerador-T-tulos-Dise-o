...
interface ThemeExplorationProps {
    ...
    allData: any; // To get pdfContent
}

const ThemeExploration: React.FC<ThemeExplorationProps> = ({ profile, topic, onTopicChange, aiResponse, onAiResponseChange, setIsAiTyping, onAiResponseComplete, allData }) => {
    ...
    useEffect(() => {
        if (devMode) {
            ...
            onTopicChange(`El impacto del diseño centrado en el desarrollador #${randomId} para acelerar los flujos de trabajo de proyectos de título asistidos por IA.`);
        }
    }, [devMode]);
    ...
    const generateAiContent = async (isIteration = false) => {
        ...
        if (!aiActive) {
            onAiResponseChange('<p>La IA está desactivada. Este es un texto de ejemplo que puedes editar.</p>');
            setBibliography('<div id="bibliografia"><h3>Bibliografía (IA Inactiva)</h3></div>');
            setIsLoading(false);
            // onAnimationComplete will be triggered by HtmlTypewriter, setting isAnimating and isAiTyping to false
            return;
        }
        ...
        try {
            ...
            const pdfContextPrompt = allData.pdfContent
                ? `\n\n--- INICIO DEL DOCUMENTO DEL ESTUDIANTE ---\n${allData.pdfContent}\n--- FIN DEL DOCUMENTO DEL ESTUDIANTE ---\nUsa el contenido de este documento como el contexto principal del trabajo que el estudiante ya ha realizado. Basa tus respuestas y sugerencias en esta información.`
                : '';
