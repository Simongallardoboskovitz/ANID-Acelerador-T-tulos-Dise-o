...
const DesignOpportunity: React.FC<DesignOpportunityProps> = ({ onAccept, profile, allData, setIsAiTyping }) => {
    ...
    useEffect(() => {
        if (devMode) {
            ...
            setGapInput(`Existe una brecha #${randomId} en la comprensión de cómo las herramientas internas de desarrollo, a menudo sin un diseño cuidado, impactan la capacidad de innovación y la calidad del trabajo de título.`);
        }
    }, [devMode]);
    ...
    const handleGenerateVariants = async () => {
        ...
        if (!aiActive) {
            setConceptualVariants(['Variante Conceptual 1 (IA Inactiva)', 'Variante Conceptual 2 (IA Inactiva)', 'Variante Conceptual 3 (IA Inactiva)']);
...
No incluyas títulos ni numeración.`;
    ...
    const handleSelectVariant = async (variant: string) => {
        ...
        if (!aiActive) {
            setDevelopedVariantText('<p>Variante desarrollada (IA inactiva). Este es un texto de ejemplo que puedes editar.</p>');
            setView('variant_developed');
            // onComplete from HtmlTypewriter will set isAnimating/isAiTyping to false
            return;
        }
...
TAREA: Desarrolla esta variante en un párrafo. Profundiza en su sentido, su alcance potencial y cómo se relaciona con el caso del estudiante. Usa HTML simple (<p>, <strong>).`;
    ...
    const handleIterateVariant = async () => {
        ...
        if (!aiActive) {
            setDevelopedVariantText('<p>Variante iterada (IA inactiva). Este es un nuevo texto de ejemplo que puedes editar.</p>');
            setView('variant_developed');
            // onComplete from HtmlTypewriter will handle the rest
            return;
        }
