...
const DisciplinaryScope: React.FC<DisciplinaryScopeProps> = ({ 
    ...
    useEffect(() => {
        if (devMode) {
            ...
            onUserInputChange(`Diseño de Interacción y Diseño de Servicios #${randomId}, enfocado en la Experiencia de Desarrollador (DevEx) para optimizar herramientas creativas.`);
        }
    }, [devMode]);
    ...
    const generateAiContent = async (isIteration = false) => {
        ...
        if (!aiActive) {
            onAiResponseChange('<p>Respuesta de IA omitida (IA inactiva). Puedes editar este contenido.</p>');
            setIsLoading(false);
