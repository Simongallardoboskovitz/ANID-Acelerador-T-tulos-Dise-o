...
const ReferenceAnalysis: React.FC<ReferenceAnalysisProps> = ({ onAccept, profile, allData, setIsAiTyping }) => {
    ...
    useEffect(() => {
        if (devMode) {
            ...
            setCategories([
                `Automated Testing Tools #${randomId}`,
                `AI Developer Experience #${randomId}`,
                `Rapid Prototyping Frameworks #${randomId}`
            ]);
        }
    }, [devMode]);
    ...
    const handleSearch = async (category: string, index: number) => {
        ...
        if (!aiActive) {
            const fallbackRef: Reference = {
                name: 'Referencia de ejemplo (IA inactiva)',
                author: 'Desarrollador',
                source: 'localhost',
                description: 'Esta es una referencia de ejemplo porque las llamadas a la IA están desactivadas.',
                relevance: 'Permite probar el flujo de la aplicación sin consumir API quota.'
            };
            setResults(prev => ({ ...prev, [category]: [fallbackRef, fallbackRef] }));
...
Prioriza fuentes como MIT Press Open, Dezeen, Core77, Behance, ArchDaily, Designboom, e Interaction Design Foundation.`;
    ...
    const handleGenerateBenchmark = async (isIteration = false) => {
        ...
        if (!aiActive) {
            setBenchmark('<h3>Benchmark (IA Inactiva)</h3><p>Análisis de benchmark omitido. Puedes editar este texto manualmente o continuar.</p>');
            setView('benchmark');
