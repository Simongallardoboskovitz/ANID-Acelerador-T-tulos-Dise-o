...
interface ProfileFormProps {
    ...
    onPdfUploaded: (content: string, fileName: string) => void;
    pdfFileName: string | null;
}

const ProfileForm: React.FC<ProfileFormProps> = ({ onProfileGenerated, setIsAiTyping, onPdfUploaded, pdfFileName }) => {
    ...
    useEffect(() => {
        if (devMode) {
            ...
            setName(`Dev User ${randomId}`);
            setPronoun('Elle');
            setPreference(`Aprender automáticamente (run #${randomId})`);
        }
    }, [devMode]);
    ...
    const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file.type !== 'application/pdf') {
            setPdfError('Por favor, sube un archivo PDF.');
            return;
        }
        ...
        reader.onload = async (e) => {
            try {
                if (!e.target?.result) throw new Error("Error reading file");
                ...
                onPdfUploaded(fullText, file.name);
            } catch (err) {
                console.error(err);
                setPdfError('No se pudo procesar el PDF. Inténtalo de nuevo.');
            } finally {
                setIsPdfLoading(false);
            }
        };
        reader.onerror = () => {
             setPdfError('No se pudo leer el archivo.');
             setIsPdfLoading(false);
        }
    ...
    const handleFormSubmit = async (e: React.FormEvent) => {
        ...
        if (!aiActive) {
            setAiWelcomeMessage(`<p>¡Hola, <strong>${name}</strong>! Bienvenido. Como la IA está desactivada, puedes continuar directamente. ¿Qué tema te parece inquietante?</p>`);
            // HtmlTypewriter will call onComplete which sets setIsAiTyping(false)
            return;
        }
...
`;
    ...
    return (
        <>
            ...
            <div className="mt-8 bg-gray-900 text-white p-6 sm:p-8 rounded-xl shadow-lg text-center">
                <h3 className="text-xl font-bold mb-4">¿Ya tienes un avance?</h3>
                <p className="mb-6">Sube tu memoria o borrador en PDF para que la IA lo use como contexto y te dé respuestas más precisas y personalizadas.</p>
                ...
                <button
                    ...
                    className="py-3 px-6 border-none rounded-lg bg-white text-black text-base font-semibold cursor-pointer hover:bg-gray-200 disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors"
                    disabled={isPdfLoading}
                >
                    {isPdfLoading ? 'Procesando PDF...' : (pdfFileName ? `Reemplazar: ${pdfFileName}` : 'Agrega tu avance (PDF)')}
                </button>
