...
const FinalReport: React.FC<FinalReportProps> = ({ allData, onAccept }) => {
    ...
    const generateReport = async (isRobustify = false) => {
        ...
        if (!isRobustify) {
            if (devMode) {
                setIsLoading(true);
                setCurrentLoadingPhrase('Generando informe en modo desarrollador...');
            } else {
        ...
        if (!aiActive) {
            setReportContent('<h1>Informe Final (IA Inactiva)</h1><p>El contenido del informe se ha omitido. Puede editar este texto para generar su propio informe.</p>');
        ...
        try {
            ...
            const pdfContextPrompt = allData.pdfContent
                ? `\n\n--- INICIO DEL DOCUMENTO DEL ESTUDIANTE ---\n${allData.pdfContent}\n--- FIN DEL DOCUMENTO DEL ESTUDIANTE ---\n**Instrucción Clave:** El siguiente documento es el trabajo previo del estudiante. Debes integrarlo de forma inteligente en el informe final, usándolo como la base principal y complementándolo con los datos generados en la plataforma. El informe final debe sentirse como una evolución del documento original.`
                : '';
...
**Datos del Proyecto (Generados en la Plataforma):**
${JSON.stringify({ ...allData, pdfContent: 'Contexto en sección separada' }, null, 2)}
